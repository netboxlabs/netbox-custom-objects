...
1. Force FK constraints on custom_objects_* tables to be non-DEFERRABLE.
   DEFERRABLE constraints queue trigger events that block subsequent ALTER
   TABLE calls (e.g. remove_field during a branch revert) with "cannot ALTER
   TABLE because it has pending trigger events".

2. Add a frozen ``db_column`` to ``CustomObjectTypeField``.  The physical
   column name is set once at creation and never updated, so renames are
   metadata-only and cannot mismatch across schemas (branch vs. main).
   Existing rows are back-filled with ``db_column = name``.
"""
